...
type (
	...
	// InfoWithRevision is the same Info, but it also has container revision.
	// Every container changing action increases revision counter.
	InfoWithRevision struct {
		Info
		Revision uint32
	}
...
func _deploy(data any, isUpdate bool) {
	...
	if isUpdate {
		...
		if version < 27_000 {
			...
			migrateToContainersWithRevision()
		}
...
func PutNamed(container []byte, signature interop.Signature,
	...
	if name != "" {
		if zone == "" {
			zone = string(storage.LocalGet([]byte(nnsRootKey)))
		}
		nnsContractAddr = interop.Hash160(storage.LocalGet([]byte(nnsContractKey)))
		domain = name + "." + zone
		needRegister = checkNiceNameAvailable(nnsContractAddr, domain)
	}

	alphabet := common.AlphabetNodes()
	netmapContractAddr := interop.Hash160(storage.LocalGet([]byte(netmapContractKey)))
	balanceContractAddr := interop.Hash160(storage.LocalGet([]byte(balanceContractKey)))
...
func CreateV2(cnr Info, invocScript, verifScript, sessionToken []byte) interop.Hash256 {
	...
	if name != "" {
		...
		nnsContract := interop.Hash160(storage.LocalGet([]byte(nnsContractKey)))
		domain := name + "." + zone
	...
	netmapContract := interop.Hash160(storage.LocalGet([]byte(netmapContractKey)))
	...
	if fee > 0 {
		balanceContract := interop.Hash160(storage.LocalGet([]byte(balanceContractKey)))
...
func Remove(id []byte, invocScript, verifScript, sessionToken []byte) {
	...
	if domain := string(storage.LocalGet(domainKey)); len(domain) != 0 { // != "" not working
...
//
// Deprecated: use [GetRevisionedInfo] instead.
func GetInfo(id interop.Hash256) Info {
	return GetRevisionedInfo(id).Info
}

// GetRevisionedInfo reads container with its revision by ID. If the container is
// missing, GetRevisionedInfo throws [cst.NotFoundError] exception.
func GetRevisionedInfo(id interop.Hash256) InfoWithRevision {
...
func tryGetInfo(id interop.Hash256) (InfoWithRevision, bool) {
...
func Alias(cid []byte) string {
	...
	return string(storage.LocalGet(append([]byte(nnsHasAliasKey), cid...)))
}
...
func CommitContainerListUpdate(cID interop.Hash256, replicas []uint8) {
	...
	for iterator.Next(oldNodes) {
		...
		if !stillInNetmap {
			...
			if reportRaw != nil {
				report := std.Deserialize(reportRaw).(NodeReport)
	...
	if spaceDiff != 0 || objsDiff != 0 {
		...
		rSummary := std.Deserialize(storage.LocalGet(summaryKey)).(NodeReportSummary)
...
func GetEACLData(id []byte) []byte {
	...
	return storage.LocalGet(append(eACLPrefix, id...))
}
...
func PutReport(cid interop.Hash256, sizeBytes, objsNumber int, pubKey interop.PublicKey) {
	...
	if reportsSummaryRaw != nil {
		summary = std.Deserialize(reportsSummaryRaw).(NodeReportSummary)
	}
	if reportRaw != nil {
		report = std.Deserialize(reportRaw).(NodeReport)
	}
	if billingStatRaw != nil {
		billingStat = std.Deserialize(billingStatRaw).(EpochBillingStat)
	}
...
func GetNodeReportSummary(cid interop.Hash256) NodeReportSummary {
	...
	cnrSummaryRaw := storage.LocalGet(key)
...
func getReportByKey(key []byte) NodeReport {
	...
	if reportRaw != nil {
		return std.Deserialize(reportRaw).(NodeReport)
	}
...
func GetBillingStatByNode(cid interop.Hash256, pubKey interop.PublicKey) EpochBillingStat {
	...
	reportRaw := storage.LocalGet(key)
	if reportRaw != nil {
		return std.Deserialize(reportRaw).(EpochBillingStat)
	}
...
func Transfer(to interop.Hash160, tokenID []byte, data any) bool {
	...
	bin := []byte(storage.LocalGet(binKey)) // Need a proper buffer here to copy() below.
	if bin == nil {
	...
	if !from.Equals(to) {
		...
		runtime.Notify("ContainerUpdated", interop.Hash256(tokenID), cnr.Revision)
		...
		if cnrSizeItem := storage.LocalGet(append([]byte{reportsSummary}, tokenID...)); cnrSizeItem != nil {
			cnrSize := convert.ToInteger(cnrSizeItem)
...
func Properties(tokenID []byte) map[string]any {
	...
	if item := storage.LocalGet(append([]byte{infoPrefix}, tokenID...)); item == nil {
		...
		cnr = std.Deserialize(item).(InfoWithRevision).Info
	}
...
func removeContainer(id []byte, owner []byte) {
	...
	if summaryRaw != nil {
		rSummary := std.Deserialize(summaryRaw).(NodeReportSummary)
...
func getContainer(cid []byte) Container {
	...
	if data != nil {
		return Container{Value: data, Sig: interop.Signature{}, Pub: interop.PublicKey{}, Token: []byte{}}
	}
...
func checkLock(cnr InfoWithRevision) string {
	for i := range cnr.Attributes {
		if cnr.Attributes[i].Key == lockAttributeName {

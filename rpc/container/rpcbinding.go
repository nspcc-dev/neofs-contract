...
func (c *ContractReader) Nodes(cID util.Uint256) (uuid.UUID, result.Iterator, error) {
	return unwrap.SessionIterator(c.invoker.Call(c.hash, "nodes", cID))
}
...
// number of result items from the iterator right in the VM and return them to
// you. It's only limited by VM stack and GAS available for RPC invocations.
func (c *ContractReader) NodesExpanded(cID util.Uint256, _numOfIteratorItems int) ([]stackitem.Item, error) {
	return unwrap.Array(c.invoker.CallAndExpandIterator(c.hash, "nodes", _numOfIteratorItems, cID))
}
...
func (c *Contract) AddNextEpochNodes(cID util.Uint256, publicKeys keys.PublicKeys) (util.Uint256, uint32, error) {
	return c.actor.SendCall(c.hash, "addNextEpochNodes", cID, publicKeys)
}

// AddNextEpochNodesTransaction creates a transaction invoking `addNextEpochNodes` method of the contract.
// This transaction is signed, but not sent to the network, instead it's
// returned to the caller.
func (c *Contract) AddNextEpochNodesTransaction(cID util.Uint256, publicKeys keys.PublicKeys) (*transaction.Transaction, error) {
	return c.actor.MakeCall(c.hash, "addNextEpochNodes", cID, publicKeys)
}
...
func (c *Contract) AddNextEpochNodesUnsigned(cID util.Uint256, publicKeys keys.PublicKeys) (*transaction.Transaction, error) {
	return c.actor.MakeUnsignedCall(c.hash, "addNextEpochNodes", nil, cID, publicKeys)
}

// CommitContainerListUpdate creates a transaction invoking `commitContainerListUpdate` method of the contract.
// This transaction is signed and immediately sent to the network.
// The values returned are its hash, ValidUntilBlock value and error if any.
func (c *Contract) CommitContainerListUpdate(cID util.Uint256) (util.Uint256, uint32, error) {
	return c.actor.SendCall(c.hash, "commitContainerListUpdate", cID)
}

// CommitContainerListUpdateTransaction creates a transaction invoking `commitContainerListUpdate` method of the contract.
// This transaction is signed, but not sent to the network, instead it's
// returned to the caller.
func (c *Contract) CommitContainerListUpdateTransaction(cID util.Uint256) (*transaction.Transaction, error) {
	return c.actor.MakeCall(c.hash, "commitContainerListUpdate", cID)
}
...
// Any fields of it that do not affect fees can be changed (ValidUntilBlock,
// Nonce), fee values (NetworkFee, SystemFee) can be increased as well.
func (c *Contract) CommitContainerListUpdateUnsigned(cID util.Uint256) (*transaction.Transaction, error) {
	return c.actor.MakeUnsignedCall(c.hash, "commitContainerListUpdate", nil, cID)
}

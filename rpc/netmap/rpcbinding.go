...
// ListNodesVersion invokes `listNodesVersion` method of contract.
func (c *ContractReader) ListNodesVersion(version *big.Int) (uuid.UUID, result.Iterator, error) {
	return unwrap.SessionIterator(c.invoker.Call(c.hash, "listNodesVersion", version))
}

// ListNodesVersionExpanded is similar to ListNodesVersion (uses the same contract
// method), but can be useful if the server used doesn't support sessions and
// doesn't expand iterators. It creates a script that will get the specified
// number of result items from the iterator right in the VM and return them to
// you. It's only limited by VM stack and GAS available for RPC invocations.
func (c *ContractReader) ListNodesVersionExpanded(version *big.Int, _numOfIteratorItems int) ([]stackitem.Item, error) {
	return unwrap.Array(c.invoker.CallAndExpandIterator(c.hash, "listNodesVersion", _numOfIteratorItems, version))
}

...
func deployContainerContractInternal(t *testing.T, e *neotest.Executor, args []any) util.Uint160 {
	...
	transferSomeGasToAccount(t, e, e.Committee)
...
// Returned invokers are for container, balance, netmap and nns contracts.
...
func TestContainerCreateV2(t *testing.T) {
	...
	assertSuccessStorage := func(t *testing.T, id cid.ID, cnrFields []stackitem.Item, cnrBytes []byte, ownerID user.ID) {
		...
		var cnrRevisioned = []stackitem.Item{
			stackitem.NewStruct(cnrFields),
			stackitem.Make(0),
		}

		require.Equal(t, stackitem.NewStruct(cnrRevisioned), gotStruct)
...
func TestContainerTransfer(t *testing.T) {
	...
	assertNotificationEvent(t, events[0], "ContainerUpdated", id[:], big.NewInt(1))
...
func TestSetAttribute(t *testing.T) {
	...
	t.Run("store and remove CORS", func(t *testing.T) {
		...
		assertNotificationEvent(t, res.Events[1], "ContainerUpdated", cID[:], big.NewInt(1))
	...
	t.Run("lock", func(t *testing.T) {
		...
		assertNotificationEvent(t, res.Events[1], "ContainerUpdated", id[:], big.NewInt(1))
